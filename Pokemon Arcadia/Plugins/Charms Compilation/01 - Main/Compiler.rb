...
module Compiler
  ...
  if PluginManager.installed?("Lin's Friend Safari") || PluginManager.installed?("Lin's BP System")
    PLUGIN_FILES += ["Charms Compilation"]
  else
    PLUGIN_FILES = ["Charms Compilation"]
  end
    ...
    PLUGIN_FILES.each do |plugin|
      ...
      pbCompilerEachPreppedLine(path) { |line, line_no|
        ...
        if line[/^\s*\[\s*(.+)\s*\]\s*$/]
          ...
          next if !line_schema
          property_value = get_csv_record($~[2], line_no, line_schema)
...
#      compile_plugin_abilities
      compile_plugin_items		if PluginManager.installed?("Lin's BP System")
...
#      compile_plugin_map_metadata
    ...
    MessageTypes.load_message_files("Data/messages.dat") if FileTest.exist?("Data/messages.dat")
